...
function select_best(crit::MaxMBUCB, h_node::POWTreeObsNode, rng)
    tree = h_node.tree
    h = h_node.node
    best_criterion_val = -Inf
    local best_node::Int
    ...
    for node in tree.tried[h]
        ...
        elseif n == 0
            ...
            criterion_value = tree.v[node] + crit.c*sqrt(ltn/ln)
        end
        if criterion_value > best_criterion_val
            best_criterion_val = criterion_value
            best_node = node
...
function select_best(crit::MaxPUCB, h_node::POWTreeObsNode, rng)
    tree = h_node.tree
    h = h_node.node
    best_criterion_val = -Inf
    local best_node::Int
    ...
    for node in tree.tried[h]
        ...
        elseif n == 0
            ...
            criterion_value = tree.v[node] + crit.c*sqrt(ltn/n)*(1-n/total_n)
        end
        if criterion_value > best_criterion_val
            best_criterion_val = criterion_value
            best_node = node
