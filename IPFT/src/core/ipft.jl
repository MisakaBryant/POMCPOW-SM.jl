...
"""
function action_info(p::IPFTPlanner, s; tree_in_info=false)
...
"""
function simulate(p::IPFTPlanner{P, S, A, SE, NA, RNG}, snode::Int, s::S, d::Int) where {P, S, A, SE, NA, RNG}
    sol = p.solver
...
"""
belief updater. 原论文使用 ObsAdaptiveParticleFilter，这里统一使用 BootstrapFilter。
"""
function POMDPs.updater(p::IPFTPlanner)
    rng = MersenneTwister(rand(p.solver.rng, UInt32))
    return BootstrapFilter(p.mdp, Int(min(10000, 10*p.solver.n_iterations, 1e7*p.solver.max_time)), rng)
end
