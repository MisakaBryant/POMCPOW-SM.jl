...
function InformationRewardBeliefMDP(pomdp::P, resampler, im, discount_information_gain, max_frac_replaced, rng=Random.GLOBAL_RNG) where {P<:POMDP}
    S = statetype(pomdp)
    b0 = resample(resampler, POMDPs.initialstate(pomdp), rng)
...
function POMDPs.gen(bmdp::InformationRewardBeliefMDP{P,R,S,B,A}, ib::InformationBelief{B}, a, rng::AbstractRNG) where {P,R,S,B,A}
    b = ib.b
    ...
    sp_o, o = @gen(:sp, :o)(bmdp.pomdp, s_o, a, rng)
    while true
        sp_o, o = @gen(:sp, :o)(bmdp.pomdp, s_o, a, rng)
    ...
    if all_terminal
        ...
        return (sp=InformationBelief{B}(resample(bmdp.resample, POMDPs.initialstate(bmdp.pomdp), rng), ib.i),
                r=[0.0, 0.0])
    ...
    ps = particles(pc)
    mpw = try
        max_possible_weight(bmdp.pomdp, a, o)
    catch
        maximum(wm)
    end
    ...
    return (sp=InformationBelief{B}(pc, ip),
            r=Array{Float64}([mean(rm,weights(wm)), d * ip - ib.i]))
end
...
function POMDPs.simulate(sim::RolloutSimulator, mdp::InformationRewardBeliefMDP{P,R,S,B,A}, policy::Policy, initialstate::InformationBelief{B}) where {P,R,S,B,A}
